//
// file: mp_gridlock_fx.gsc
// description: clientside fx script for mp_gridlock: setup, special fx functions, etc.
...
{
	fxanim_pennants = GetEntArray( localClientNum, "fxanim_mp_grid_pennants_mod", "targetname" ); 

	for ( i = 0; i < fxanim_pennants.size; i++ )
	{
		fxanim_pennants[i] thread fxanim_think( localClientNum, "fxanim_mp_grid_pennants", level.waits[i], level.speeds[i] );
	}

	fxanim_roofvents = GetEntArray( localClientNum, "fxanim_mp_roofvent_mod", "targetname" ); 

	for ( i = 0; i < fxanim_roofvents.size; i++ )
	{
		fxanim_roofvents[i] thread fxanim_think( localClientNum, "fxanim_mp_roofvent_snow", level.waits[i], level.speeds[i] );
	}

	fxanim_wires = GetEntArray( localClientNum, "fxanim_gp_wirespark_med", "targetname" );

	for ( i = 0; i < fxanim_wires.size; i++ )
	{
		fxanim_wires[i] thread fxanim_wire_think( localClientNum, "fxanim_gp_wirespark_med_anim", level.waits[i], level.speeds[i] );
	}
}
...
fxanim_wire_think( localClientNum, anim_index, wait_time, speed )
{
	...
	for ( ;; )
	{
		...
		{
			PlayFxOnTag( localClientNum, level.gridlock_fx[ "fx_elec_spark" ], self, "long_spark_06_jnt" );
		}
		else if ( anim_index == "fxanim_gp_wirespark_med_anim" )
		{
			PlayFxOnTag( localClientNum, level.gridlock_fx[ "fx_elec_spark" ], self, "med_spark_06_jnt" );
		}
	}
}
...
main()
{
	clientscripts\mp\createfx\mp_gridlock_fx::main();
