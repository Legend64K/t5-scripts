...
play_rotor_fx(localClientNum) // self == the helicopter
{
	self endon( "entityshutdown" ); 
	level endon( "save_restore" ); 
	...
	//-- PLAY THE STARTUP SEQUENCE IF NEEDED
	if( IsDefined(self.csf_stationary_rotor) && !self.csf_stationary_rotor)
	{
		...
		//-- TODO: figure out if there is a better way to wait here
	...
	// setup the running main rotor fx
	if( isdefined( self.rotorMainRunningFxName ) && self.rotorMainRunningFxName != "" )
	{
		...
		if( IsDefined(self.rotorMainRunningFx) )
		{
			...
			self.rotorMainfxHandle_saved = self.rotorMainfxHandle;
			//self thread loop_rotor_fx(localClientNum, self.rotorMainRunningFx, "main_rotor_jnt");
		}
		else
		{
	...
	// setup the running tail rotor fx
	if( isdefined( self.rotorTailRunningFxName ) && self.rotorTailRunningFxName != "" )
	{
		...
		{
			// tail running is a looping fx
			self.rotorTailfxHandle = PlayFXOnTag(localClientNum, self.rotorTailRunningFx, self, "tail_rotor_jnt");
			self.rotorTailfxHandle_saved = self.rotorTailfxHandle;
			//self thread loop_rotor_fx(localClientNum, self.rotorTailRunningFx, "tail_rotor_jnt");
		}
		else
		{
...
play_rotor_fx_stationary(localClientNum)  // self == the helicopter
{
	self endon( "entityshutdown" ); 
	level endon( "save_restore" ); 
	...
	if( isdefined( self.rotorTailIdleFxName ) && self.rotorTailIdleFxName != "" )
	{
		...
		{	
// 			println("Client: _vehicles.csc - play_rotor_fx_stationary() - self.rotorTailfxHandle = " + self.rotorTailfxHandle );
// 			println("Client: _vehicles.csc - play_rotor_fx_stationary() - self.rotorTailIdleFx = " + self.rotorTailIdleFx );
// 			println("Client: _vehicles.csc - play_rotor_fx_stationary() - localClientNum = " + localClientNum );
...
loop_rotor_fx(localClientNum, effect, attachTag) // self == the helicopter
{
	self endon( "entityshutdown" ); 
	level endon( "save_restore" ); 
	...
	// check for a custom rotor fx function so we can do other things when we want
...
vehicle_flag_switch_rotor_fx(localClientNum, set, newEnt)
{
	if(set)
	{
		if(IsDefined(newEnt) && newEnt)
		{
			//-- This is for the special case that the helicopter had existed before with this flag set
			//    but left the snapshot and is now back as a new entity
