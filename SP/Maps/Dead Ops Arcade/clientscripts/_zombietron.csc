...
//	clientscripts\_callbacks::AddPlayWeaponDeathEffectsCallback( "ray_gun_upgraded_zm", ::ray_gun_death_effects );
...
lightning(localClientNum, set, newEnt)
{
	...
	if( set )
	{
		intensity_low = 10;
		intensity_high = 30;
		min_flash_time = 0.3; 				 // how long the flashing lasts
		max_flash_time = 0.7;
		flash_wait_time_min = 0.05;			// time between individual flashes
...
init_client_flag_callbacks()
{
	...
	register_clientflag_callback("scriptmover", level._ZT_SCRIPTMOVER_CF_POWERUP, ::powerup);	// Set when spawned - clear when grabbed
	register_clientflag_callback("scriptmover", level._ZT_SCRIPTMOVER_CF_ELEC_TRAP, ::electrap_handler);
	register_clientflag_callback("scriptmover", level._ZT_SCRIPTMOVER_CF_POWERUP_PICKUP, ::powerup_pickup);
...
smartbomb_handler(localClientNum, set, newEnt)
{
	...
	if (IsDefined( self.tank )) //something about the tank screws the player origin up.
	{
...
powerup_pickup(localClientNum, set, newEnt)
{
	if(localClientNum != 0)
	{
		return;
