...
init_client_flags()
{
	...
	level._ZT_SCRIPTMOVER_CF_POWERUP = 1;	// Set when spawned - clear when grabbed.
	level._ZT_SCRIPTMOVER_CF_ELEC_TRAP = 2;
	level._ZT_SCRIPTMOVER_CF_POWERUP_PICKUP = 3;
...
arena_initialize()
{
	//if you change this order, be sure to update the devgui challenges
	level.arenas = [];
	level.arenas[level.arenas.size] = "island";		//round 1-4
	level.arenas[level.arenas.size] = "town";			//round 5-8			//Electrical Spires introduced
	level.arenas[level.arenas.size] = "prison";		//round 9-12		//sergei round (9)
	level.arenas[level.arenas.size] = "temple";		//round 13-16		//quad round (13)
	level.arenas[level.arenas.size] = "factory";	//round 17-20		//miss daisy round (17)
	level.arenas[level.arenas.size] = "rooftop";	//round 21-24		//engineer round (23)
	level.arenas[level.arenas.size] = "street";		//round 25-28		//Red Barrel Avalanche(25)
	level.arenas[level.arenas.size] = "bunker";		//round 29-32		//yeti round (30)
	level.arenas[level.arenas.size] = "snow";			//round 33-36		//dog round (34)
	level.arenas[level.arenas.size] = "jungle";		//round 37-40		//ape round (40)
	level.current_arena 						= 0;
	level.arena_laps 								= 0;
	level.raw_round									= 0; // used for MTU reporting.
...
init_difficulty()
{
	...
	while(1)
	{
		...
		//do some shit here if you need to.
	}
}
...
//	{
//		players[i] SetClientDvars("cg_scoreboardFont",4,"cg_scoreboardTextOffset",0.60);
//	}
	wait 0.2;
	move_players_to_start();	
...
mini_boss_spawn_think()
{
	...
	while(1)
	{
		...
		if (RandomInt(100) < (level.zombie_vars["mini_boss_spawn_percentage"]+(level.round_number*GetPlayers().size) ) )
		{
			...
			if ( GetPlayers().size == 4 )//spawn a dropper to thwart door camping..
			{
				spawn_locations = level.current_spawners[level.current_wave.spawn_side];
				spawn_point = spawn_locations[RandomInt( spawn_locations.size )]; 
...
#/
	...
	for( wave = 0; wave < level.spawn_sequence.size; wave++ )
	{
		...
		if ( flag("boss_is_spawned") )
		{
			...
			{
				if ( wave == level.spawn_sequence.size - 1) //boss is still alive, but we're about to stop spawning fodder; this makes the boss easy
				{
					wave -= RandomIntRange(4,10);//rewind a little bit.
					if ( wave < 0 ) wave = 0;//this shouldnt happen given the small subtraction above, but doesn't hurt to be safe.
				}
			}
		}
		if ( isDefined(level.desiredRound) )
	...
	//failsafe; Kill everything in 20seconds;
	//level thread kill_all_the_mfn_zombies(20);
	wait 1;
	zombies = GetAISpeciesArray( "axis", "all" );
...
player_basic_needs()
{
	...
	{	
		...
		//Make sure that the player has at least one speed boost
...
{
	ClearAllCorpses();
	player_basic_needs();//give the player the 'basics' as needed
...
begin_game()
{
	...
	for (i = 0; i < players.size; i++)
	{
		...
		players[i] giveachievement_wrapper( "SP_ZOM_FLAMINGBULL" );		//flamingbull is actually 'Insert Coin' (You found DOA , yay)
	...
	while( 1 )
	{
		...
		else
		{
			...
			// need to wait till all zombies are dead
			...
			if( level.arena_round_number <= level.zombie_vars["zombie_arena_rounds"] )
			{
				if ( !isDefined(level.desiredRound) )
				{
					...
					if ( maps\_zombietron_fate::time_to_choose_fate() )
					{
						...
						level notify( "change_music_state" );
						maps\_zombietron_fate::choose_fate_round();//blocking
					}
				}
				update_next_arena();
			}
            ...
            level notify( "change_music_state" );
						...
						level.round_number--; //HACK Allen Wu needs the round not to increment
...
end_game()
{
	...
	// give the last player some time to see his own death
	wait 2;
...
//	iprintlnbold("all_players_dead has been set");
	update_leaderboards();
	wait 0.05;
	ClearAllCorpses();
	...
	else
	{
		...
		// Hack for 30 sec black screen when quitting the game when the game is already over
		if( IsSplitScreen() )
		{
			while( 1 )
			{
...
move_players_to_start()
{
	...
	for (i = 0; i < players.size; i++)
	{
		...
		// This actually prevents the camera lerping
...
steal_life_from(source,dest)
{
	...
	//give the donor a little tickle
	reward = undefined;
	model  = undefined;
	number = 1;
...
player_respawn()
{
	...
	{
		self.stolen_life = undefined;//somebody donated a life to us.  Needed a second variable so that we avoid the case that somebody else steals the newly donated life (due to misc waits etc)
	}
	self player_respawn_now();
...
open_exit( trigger, objective_id )
{
	...
	//Objective_String( objective_id, &"EXIT" );
...
open_exits(specific)
{
	...
	if( level.arena_round_number == level.zombie_vars["zombie_arena_rounds"])
	{
		// act like we are going to a boss fight and only provide one exit
		num_exits = 1;
	}


	level.msg_t3.alpha 			= 0;
	//this is the bonus room selection process;
	//only occurs if exits are 2 or more open
	//no back to back magical exits
	...
	// script_parameters is "top", "bottom", "left", or "right"
...
spawn_teleporter(boss_battle)
{
	...
	if (boss_battle)//play victory movie
	{
		wait 1;
...
setup_random_environment()
{
	...
	for (i = 0; i < players.size; i++)
  {
		...
		{
			players[i] SetClientDvars("phys_buoyancy",1,"phys_ragdoll_buoyancy",0);
		}
	}
	
	// kill all of the previous fx
...
hide_model()
{
	...
	{
		if( self.spawnflags & 1 )	// dynamic path
		{
			self ConnectPaths();
		}
		self.hidden = true;
...
init_function_overrides()
{
	...
	//level.global_damage_func		= maps\_zombietron_spawner::zombie_damage; 
	//level.global_damage_func_ads	= maps\_zombietron_spawner::zombie_damage_ads;
...
player_damage_override( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, modelIndex, psOffsetTime )
{
	...
	if( isDefined( eAttacker ) )
	{
		...
		if( (isDefined( eAttacker.is_zombie ) && eAttacker.is_zombie) )
		{
			...
			// Mark M (09/16/09)  Temp melee damage override
...
Callback_PlayerKilledZT( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	{
		self.zombification_time = GetTime() - level.round_start_time; //set time when player died
	}
...
detect_and_change_music_states()
{
    ...
    while(1)
    {
        ...
        //IPrintLnBold( "Change Music State" );
