...
init()
{
	...
	level.pickup_types 		= []; //the MASTER list(everything is in it)
	level.pickup_weapons 	= []; //subset of just weapons
	
	//ALL Pickups should be in here; This is the MASTER list.
	...
	level.pickup_types[level.pickup_types.size] = "mini_turret";  //two turrets
	level.pickup_types[level.pickup_types.size] = "mini_turret";  //three turrets
	level.pickup_types[level.pickup_types.size] = "monkey";
	level.pickup_types[level.pickup_types.size] = "spinning_blockers";
	level.pickup_types[level.pickup_types.size] = "tesla_blockers";  //electric coconuts for James! ;)
	level.pickup_types[level.pickup_types.size] = "stun_blockers";
	level.pickup_types[level.pickup_types.size] = "double_shot"; 	//1 chicken
	level.pickup_types[level.pickup_types.size] = "double_shot";	//2 chickens. yes this is right.
	level.pickup_types[level.pickup_types.size] = "double_shot";	//3 chickens. yes this is right.
...
pickup_bomb_on_player(player)//self = pickup
{
	...
	while(1)
	{
		if ( self.origin[2] < player.origin[2] )//wtf
		{
...
spawn_uber_prize(barpoints,spawn_point)
{
	...
	scale *= 0.5; //the model itself is already kind of big.
	if ( scale < 1 ) scale = 1;
	if ( scale > 5 ) scale = 5;
...
/*	if ( model == "zombietron_ruby" )
	{
		PlayFxOnTag( level._effect["red_gem"], pickup, "tag_origin" ); 
	}
	else
	if ( model == "zombietron_sapphire" )
	{
		PlayFxOnTag( level._effect["blue_gem"], pickup, "tag_origin" ); 
	}
	else
	if ( model == "zombietron_diamond" )
	{
		PlayFxOnTag( level._effect["white_gem"], pickup, "tag_origin" ); 
	}*/
	
	trigger = spawn( "trigger_radius", spawn_point, 0, 40, 128 );
//	pickup PlayLoopSound( "zmb_pickup_money_shimmer" );
	pickup thread wait_for_pickup( type, trigger );
...
get_random_pickup_type()
{
	...
	pu = level.avail_pickups [ 0 ];//already randomized
...
can_spawn_pickup( type )
{
	...
	if ( type == "mini_turret" )
	{
		if ( level.magical_exit_taken ) //turret is useless in the bonus room
		{
			return false;
		}
		if ( !can_spawn_turret() )
	...
	if ( type == "monkey" && level.magical_exit_taken )//monkey is useless in the bonus room
	{
		return false;
	}
	
	if ( type == "heli" && level.arenas[level.current_arena] == "prison" )//prison creates issues for heli when it wants to exit arena due to height map
	{
	...
	//island has vehicle clip around water which messes up the exits.
	if ( (type == "heli" || type == "tank") && level.arenas[level.current_arena] == "island" )
	{
		return false;
	}
	
	// don't allow tanks and spinning_blockers to be spawned at the same time, physics problems
...
special_pickup_idle(type)//self = picukp
{
	if (type=="double_shot")
	{
...
adjust_uber_prize_for_tank( player )
{
	if( IsDefined( player.tank ) && IsDefined( self.is_uber_prize ) )
	{
		dir = VectorNormalize( self.origin - player.tank.origin );

		self.origin = self.origin + dir * 50;
		wait 0.05;
...
{
	// allow the pickup to show up before grabbing it
	wait .4;	
	...
	if( IsDefined(player) )
	{
		...
		else if ( type == "money" )
		{
			...
			self adjust_uber_prize_for_tank( player );
			self setclientflag (level._ZT_SCRIPTMOVER_CF_POWERUP_PICKUP);
			self move_pickup_to_score( type, player );
		}
		else if ( type == "extra_life" || type == "extra_life_directed" )
		{
			if ( type == "extra_life" )
			{	// extra life goes to all players
				players = get_players();
				for( i = 0; i < players.size; i++ )
				{
					player = players[i];
	...
	self clearclientflag(level._ZT_SCRIPTMOVER_CF_POWERUP);  // Clear when grabbed.
	self notify("picked_up");
...
weapon_pickup_update()
{
	...
	while( time_left > 0 )
	{
		...
		// hack for weapon meter, self.headshots gets sent across the network
...
spinning_blockers_timeout( org )
{
	...
	//self thread delete_sound_ent( sound_ent, "new_spinners_pickup" );
...
{
	self waittill_any( "new_spinners_pickup","disconnect","player_died");//,"veh_activated" );
	self notify("new_spinners_pickup");
...
spinning_blockers_update()
{
	...
	//org LinkTo( self, "tag_origin" );	// can't link and rotate
...
tesla_blockers_timeout( org )
{
	...
	//self thread delete_sound_ent( sound_ent, "new_tesla_pickup" );
...
{
	self waittill_any( "new_tesla_pickup","disconnect","player_died");//,"veh_activated" );
	self notify( "new_tesla_pickup" );
...
//			PlayFxOnTag( level._effect["fate_explode"], guy, "j_head" ); 
...
stun_blockers_timeout( org )
{
	...
	//self thread delete_sound_ent( sound_ent, "new_stun_pickup" );
...
{
	self waittill_any( "new_stun_pickup","disconnect","player_died");//,"veh_activated" );
	self notify("new_stun_pickup");
...
double_shot_fire( orb, weapon )
{
	...
	setweapon( orb, weapon ); // It will be better to move this outside, so we dont call it every time.
	fire( orb, self );
}
...
{
	orb endon( "death" );
	if ( isDefined(fated) )//a dude with friendship fate doesn't lose chicken when dying
	{
...
double_shot_update()
{
	...
	// Have a second orb follow the first orb
	...
	while( GetTime()< orb.timeout && isDefined(orb) )
	{
		...
		if( Distance2dSquared( orb.follow_ent.origin, orb.follow_points[follow_index] ) > 4*4 )
		{
			...
			if( IsPlayer( orb.follow_ent ) )
			{
				...
				// check if we have the crow and update the follow ent
				if( IsDefined(self.fate_shot_orb) )	
				{
					orb.follow_ent = self.fate_shot_orb;
				}
			...
			//orb.origin = orb.follow_points[next_index];
...
fated_double_shot_update(model)
{
   ...
   orb PlaySound( "zmb_dblshot_spawn" );
	...
	//give the pet your color
	//PlayFxOnTag( level._effect[self.light_playFX], self.fate_shot_orb, "tag_origin" ); 
...
heli_rocket_loop( mini_heli )
{
	...
	while( 1 )
	{
		...
		if( IsDefined(driver) && driver AttackButtonPressed() )
		{
			...
			//PlayFxOnTag( level._effect[ "" ], mini_heli, fire_tag );
...
/*	origin = self GetTagOrigin( "j_head" );
	...
	CreateDynEntAndLaunch( level.head_model, origin, angles, contactPoint, launchDir, anim._effect["animscript_gibtrail_fx"] ); */
...
//				guy LaunchRagdoll( launch_dir, "head" ); 
				guy StartRagdoll( 1 );
				guy.launched = true;
...
heli_pickup_update( pickup )	// self is player
{
...
tank_pickup_update( pickup )	// self is player
{
	self EnableInvulnerability();
...
spawn_mines()
{
	...
	for( i = 0; i < count; i++ )
	{
		...
		// Call this twice to get better positions
...
update_drop_booster()
{
	...
	while( 1 )
	{
		...
		if ( IsAlive(self) && !level.in_intermission && !isDefined(self.tank) && !isDefined(self.heli) && self jumpButtonPressed() )//SprintButtonPressed() )//self meleebuttonpressed() )  
		{
			if( isDefined(self.boosters) && self.boosters > 0 )
			{
				self.rhino_deaths = 0;
				playsoundatposition( "zmb_speed_boost_activate", self.origin );
	...
	while( 1 )
	{
		if( IsAlive(self) && !level.in_intermission && self fragButtonPressed() )
		{
			if( isDefined(self.bombs) && self.bombs > 0 )
			{
				...
				ClearAllCorpses();
				//PlaySoundAtPosition( "zmb_nuke_incoming", self.origin );
				wait .3;
				//PlaySoundAtPosition( "zmb_nuke_impact", self.origin );
			//	PlayFx( level._effect["bomb"], player_org ); 
				//EarthQuake( 1.0, 0.8, player_org, 1000 ); 
...
/*				for( i = 0; i < zombies.size; i++ )
				{
					...
					dist_squared = DistanceSquared( guy.origin, player_org );
					if( dist_squared < 130 * 130 )
					{
						guy DoDamage( guy.health + 1, player_org, self, undefined, "explosive" );
						guy StartRagdoll( 1 );
						guy LaunchRagdoll( (0,0,270) );
						guy.launched = true;
						guy thread animscripts\zombie_death::flame_death_fx();
					}
					else if( dist_squared < 360 * 360 )
					{
						...
						dir = VectorNormalize( guy.origin - player_org );
						dir = dir * 100;
						dir = dir + (0,0,120);
						guy LaunchRagdoll( dir );
						guy.launched = true;
						guy thread animscripts\zombie_death::flame_death_fx();
					}
					else if( dist_squared < 1200 * 1200 )
					{
						guy DoDamage( guy.health + 1, player_org, self, undefined, "burned" );
						guy thread animscripts\zombie_death::flame_death_fx();
					}
				} */
...
turret_pickup_update(player)   //self = level
{
...
{
	self UseAnimTree(#animtree);
	self SetAnim(%fxanim_zombies_crow_fly_anim);//loops
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
