...
do_lightning_loop()
{
	...
	//while ( !level.teleporter_spawned )
	while ( 1 )
	{	
		wait( RandomFloatRange(3,20) );	// time solid state lasts
...
// Anything after can be the sundirection tweaks and other lighting based scripting changes.
setup_random_sun()
{
	// default 
	dir = "-45 135 0";
	...
	//ISLAND 1-4
		if ( level.current_arena == maps\_zombietron_main::arena_findIndexByName("island") )
		{
			if( level.arena_round_number == 1 )
			{
				...
				exposure = "1.57";
                light = "9";
				//VisionSetNaked( "zombietron_morning_island", 0 );
			}
...
// BUNKER 29-32
		if ( level.current_arena == maps\_zombietron_main::arena_findIndexByName("bunker") )//snow map
		{
			if( level.arena_round_number == 1 )
			{	
				exposure = "0.76";
	...
	// Use this for the room of fate
	// This is done in _zombietron_fate.gsc -> choose_fate_round
...
do_2d_rain_audio()
{
    ...
    level waittill_any( "end_lightning", "all_players_dead" );
