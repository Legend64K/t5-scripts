...
{
	waitforclient(0);
	init_filter_hazmat( getlocalplayers()[0] );
...
gasmask_off()
{
	while( true )
	{
		...
		disable_filter_hazmat( GetLocalPlayers()[0], 0, 0 );
...
gasmask_on_pristine()
{
	while( true ) 
	{
		...
		enable_filter_hazmat( GetLocalPlayers()[0], 0, 0, 1 /*stage 1 ( no crack )*/, 1.0 );		
...
gasmask_on_cracked_1()
{
	while( true ) 
	{
		...
		enable_filter_hazmat( GetLocalPlayers()[0], 0, 0, 2 /*stage 2 ( cracked )*/, 1.0 );
...
gasmask_on_cracked_2()
{
	while( true ) 
	{
		...
		enable_filter_hazmat( GetLocalPlayers()[0], 0, 0, 3 /*stage 3 ( cracked more )*/, 1.0 );
...
gasmask_on_cracked_3()
{
	while( true ) 
	{
		...
		enable_filter_hazmat( GetLocalPlayers()[0], 0, 0, 4 /*stage 4 ( cracked more )*/, 1.0 );
...
gasmask_breathing()
{
	...
	while( true )
	{
		set_filter_gasmask_breath(GetLocalPlayers()[0], 0, 0, breath_strength );
		wait(.05);
