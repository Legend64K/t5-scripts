...
/*	waitforallclients();
	...
	{
		players[i] thread coast_fog_triggers_init(i);
		players[i].inside = false;
	}	*/
}
...
// Front of ship (after catwalk) fog change
...
// Back of ship (inc. interiors) fog change
...
coast_fog_blizzard(localClientNum, set,newEnt)
{
	...
	{
		level.blizzard = true;
		if(IsDefined(self.inside) && self.inside == 1) // self is player.
		{
