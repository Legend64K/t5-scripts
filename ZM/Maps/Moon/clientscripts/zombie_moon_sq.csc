...
dte_watcher()
{
	...
	{
		self thread clientscripts\_zombiemode::zombie_vision_set_apply( level._dte_vision_set, level._dte_vision_set_priority, 0.1, lcn);
	}
	...
	for(lcn = 0; lcn < level._ctt_num_players; lcn ++)
	{
		self thread clientscripts\_zombiemode::zombie_vision_set_remove( level._dte_vision_set, 6.0, lcn );	
	}
	...
	for(i = 0; i < getlocalplayers().size; i ++)
	{
		...
		player clientscripts\zombie_moon_fx::moon_vision_set( "dte", player._previous_vision, i, 1.0);
	}
}
