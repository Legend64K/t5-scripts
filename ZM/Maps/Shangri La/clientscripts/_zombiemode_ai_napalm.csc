...
init_napalm_zombie()
{
	...
	//magic numbers synched with gsc side in _zombiemode_ai_napalm.gsc
...
player_napalm_radius_overlay_fade()
{
	...
	while ( 1 )
	{
		...
		//If there isn't a napalm or he is wet fade out screen fx
		...
		//Don't screen fx to pop
		delta = Clamp(frac - prevFrac, -.1,.1);
		frac = prevFrac + delta;
		
		prevFrac = frac;
		//set the overlay's alpha to frac
		SetSavedDvar("r_flameScaler",frac);
		wait(0.1);
	}
}

// If the player can't see the napalm zombie then return true so the full screen effect is removed
